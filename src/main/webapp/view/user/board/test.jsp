<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>Chi tiết bảng</title>
    <link rel="stylesheet" href="/css/user/board/detail_board.css">
</head>
<body>
<header>
    <jsp:include page="../account/menubar.jsp"/>
</header>
<main style="display: flex;overflow: hidden;">
    <div class="home-left">
        <jsp:include page="../account/home_left.jsp"/>
    </div>
    <div class="homeRight">
        <div class="title_bar">Tiêu đề</div>
        <div class="board_lists">
            <ol class="list-item">
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>
                <li class="item">
                    <div class="detail-list">
                        <div>Tiêu đề thẻ</div>
                        <ol>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjf</li>
                            <li>hihiahafsd afhsdlkf flaksflskdfjsdf aflkdsjffsdalfkj faskdlj fjalks faksj faslk</li>
                            <li>Pham Thi Kim Thuy sinh vien nam hai truong Dai hoc Thanh Do ahihi</li>
                            <li>Pham Thi Ngoc Thom sinh vien nam tu hoc vien mat ma</li>
                            <li>Nguyen Thanh Tung 28 tuoi designer cua cong ty Mac Adjust tinh Tay Nam Quang Dong Nga
                            </li>
                        </ol>
                    </div>
                </li>


            </ol>
        </div>
    </div>
</main>
</body>
</html>


<style>
    header {
        position: sticky;
        top: 0;
        z-index: 1000;
    }

    main {
        display: flex;
    }

    .home-left {
        position: sticky;
    }

    .homeRight {
        height: 95vh; /* Chiều cao toàn màn hình */
        background: #3179BA;
        display: flex;
        flex-direction: column;
    }

    .board_lists {
        flex: 1;
        display: flex;
        overflow: hidden; /* Ngăn chặn tràn nội dung */
    }

    .list-item {
        overflow-x: auto; /* Thanh cuộn ngang chỉ trong phần này */
        display: flex;
        max-width: 100%; /* Không tràn ra ngoài */
        padding: 10px;
    }

    .item {
        flex: none;
        min-width: 300px; /* Đảm bảo không bị co nhỏ */
        height: 100%;
        background: #3FB5CC;
        margin-right: 10px;
        text-align: center;
        color: white;
        padding: 10px;
        box-sizing: border-box;
        display: flex;
        flex-direction: column;
    }

    .detail-list {
        flex: 1; /* Chiếm hết phần còn lại của item */
        display: flex;
        flex-direction: column;
        background: white;
        color: black;
        padding: 10px;
        box-sizing: border-box;
        text-align: left;
        overflow-y: auto; /* Cuộn dọc nếu nội dung quá dài */
    }

    .detail-list ol {
        width: 100%;
        list-style: none;
        padding: 0;
    }

    .detail-list ol li {
        border: 1px solid #ccc;
        padding: 15px;
        margin-bottom: 5px;
        word-wrap: break-word; /* Xuống dòng nếu văn bản quá dài */
        overflow-wrap: break-word;
        white-space: normal; /* Ngăn chặn nội dung tràn ra ngoài */
    }

</style>