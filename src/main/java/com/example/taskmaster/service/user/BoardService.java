...
public class BoardService implements IBoardService{
    ...
    public List<Board> getAllBoardInGroup(int groupId, boolean sortType) {
        String query;
        if (sortType) {
            query = "select * from boards left join board_backgrounds on boards.background_id = board_backgrounds.background_id where boards.group_id = ?  order by boards.title";
        } else {
            query = "select * from boards left join board_backgrounds on boards.background_id = board_backgrounds.background_id where boards.group_id = ?  order by boards.title desc";
        }
    ...
    public List<Board> searchBoardsByName(int groupId, String keyword) {
        String query = "select * from boards left join board_backgrounds on boards.background_id = board_backgrounds.background_id where boards.group_id =  ? and boards.title like concat('%', ? , '%');";
        List<Board> boards = new ArrayList<>();
        try (Connection connection = ConnectDatabase.getConnection()) {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            preparedStatement.setInt(1, groupId);
